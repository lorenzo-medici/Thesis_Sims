...
from no_disturbance_engine import run_simulation
from utils import *
...
def setup_screen(screen: Surface | SurfaceType) -> None:
...
def draw_receiver(x: tuple[float, float], screen: Surface | SurfaceType, color=YELLOW) -> None:
...
def sensor_reading(receiver_pos: tuple[float, float], target_pos: tuple[float, float]) -> reading:
...
def compute_iota(distances: reading) -> tuple[float, float]:
    d1, d2, d3 = distances
...
def draw_iota(x: tuple[float, float], screen: Surface | SurfaceType, color=RED) -> None:
    pygame.draw.circle(screen, color, display_pos(x), 5)
...
if __name__ == '__main__':
    ...
    run_simulation(setup_screen,
                   ...
                   compute_iota,
                   draw_iota)
    # run_simulation(setup_screen: Callable[[Surface | SurfaceType], None],
    ...
    # draw_iota: Callable[[x_state], None]
    # )
